...
async function runSystemAudit() {
  ...
  posts.forEach(p => {
    if (p.imageUrl) {
      ...
      imageUsageMap[img].push({ type: 'local', id: p.id, title: p.title });
    }
  });
  ...
  Object.entries(imageUsageMap).forEach(([img, uses]) => {
    if (uses.length > 1) {
      ...
      issues.push({
        ...
        message: `Ảnh "${img.substring(img.lastIndexOf('/') + 1)}" bị dùng trùng lặp trong ${uses.length} bài viết.`,
        posts: uses
      });
    }
  });
  ...
  for (let i = 0; i < allAuditedPosts.length; i++) {
    for (let j = i + 1; j < allAuditedPosts.length; j++) {
      ...
      if (t1 && t2) {
        ...
        if (titleSim > 0.65) {
          duplicateTitlePairs.push({
            post1: { id: p1.id, title: p1.title },
            post2: { id: p2.id, title: p2.title },
      ...
      // Body Content check
      const b1 = (p1.content || '').substring(0, 2000);
      const b2 = (p2.content || '').substring(0, 2000);
      if (b1 && b2) {
  ...
  const auditReport = {
    ...
    summary: {
      ...
      healthScore: Math.max(0, 100 - (activeDuplicateContent.length * 15) - (duplicateTitlePairs.length * 10) - (localDuplicateImages.length * 5) - (invalidLandingPosts.length * 15) - (lowSeoPosts.length * 5))
    },
