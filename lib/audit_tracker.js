...
async function fetchRealGoogleRank(keyword, domain = 'xulynuochoasen.com') {
  ...
  const apiKey = config.serperApiKey || (config.googleApiKey && config.googleApiKey.length === 40 ? config.googleApiKey : '');

  if (apiKey) {
    return new Promise((resolve) => {
      const data = JSON.stringify({ q: keyword, gl: 'vn', hl: 'vi', num: 100 });
      const options = {
...
// Helper: append one history snapshot and prune entries older than 90 days
function appendHistory(item, position, isLive) {
  if (!Array.isArray(item.history)) item.history = [];
  const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
  // Avoid duplicate entry for same date (keep latest)
  item.history = item.history.filter(h => h.date !== today);
  item.history.push({ date: today, position, isLive: !!isLive });
  // Prune entries older than 90 days
  const cutoff = new Date();
  cutoff.setDate(cutoff.getDate() - 90);
  item.history = item.history.filter(h => new Date(h.date) >= cutoff);
  // Keep history sorted oldest → newest
...
async function runRankCheck() {
  ...
  if (config.serperApiKey && config.isLiveMode !== false) {
    // Live Google SERP check — scan ALL keywords (no arbitrary limit)
    for (let i = 0; i < rankings.length; i++) {
      ...
      try {
        ...
        if (liveResult && liveResult.isLive) {
          ...
          appendHistory(item, item.position, true);
        }
      } catch (err) {
    ...
    rankings.forEach(item => {
      ...
      item.isLive = false;
      item.lastChecked = now;
      appendHistory(item, item.position, false);
    });
  }
...
async function searchSerperLive({ query, domain = 'xulynuochoasen.com', num = 100 }) {
  const config = getGoogleConfig();
  const apiKey = config.serperApiKey || (config.googleApiKey && config.googleApiKey.length === 40 ? config.googleApiKey : '');

  if (!apiKey) {
    throw new Error('Chưa cấu hình Serper API Key trong hệ thống.');
  }

  return new Promise((resolve, reject) => {
    const data = JSON.stringify({ q: query, gl: 'vn', hl: 'vi', num: num, autocorrect: true });
    const options = {
      hostname: 'google.serper.dev',
      path: '/search',
      method: 'POST',
      ...
      res.on('end', () => {
        try {
          ...
          resolve({
            ...
            query,
            domain,
            parameters: { gl: 'vn', hl: 'vi', num: num },
            foundRank,
            targetUrl,
          ...
          reject(new Error('Lỗi phân tích kết quả Serper: ' + e.message));
        }
      });
    });

    req.on('error', (e) => reject(new Error('Lỗi kết nối Serper: ' + e.message)));
    req.write(data);
    req.end();
  });
}
