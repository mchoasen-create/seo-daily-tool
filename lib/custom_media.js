...
function getCustomMedia() {
  ...
  try {
    if (fs.existsSync(KHO_1_FILE) && fs.existsSync(KHO_2_FILE) && fs.existsSync(KHO_3_FILE)) {
      const k1 = JSON.parse(fs.readFileSync(KHO_1_FILE, 'utf8') || '[]');
      const k2 = JSON.parse(fs.readFileSync(KHO_2_FILE, 'utf8') || '[]');
      const k3 = JSON.parse(fs.readFileSync(KHO_3_FILE, 'utf8') || '[]');
...
const WP_ACTIVE_FILE = path.join(BASE_DIR, 'data/wp_active_images.json');

// Helper to normalize image filename (e.g. media_1788941577143_fwrm-1.jpeg -> media_1788941577143_fwrm.jpeg)
function cleanImageName(urlOrPath = '') {
  if (!urlOrPath) return '';
  const base = path.basename(urlOrPath).toLowerCase();
  return base.replace(/-\d+x\d+(\.[a-z]+)$/i, '$1').replace(/-\d+(\.[a-z]+)$/i, '$1');
}

// Return set of clean image filenames that are LOCKED on published WordPress posts or published posts in posts.json
function getLockedImagesSet() {
  const locked = new Set();

  // 1. From WP Active Images cache
  try {
    if (fs.existsSync(WP_ACTIVE_FILE)) {
      const wpMap = JSON.parse(fs.readFileSync(WP_ACTIVE_FILE, 'utf8') || '[]');
      if (Array.isArray(wpMap)) {
        wpMap.forEach(fn => locked.add(cleanImageName(fn)));
      } else if (typeof wpMap === 'object' && wpMap !== null) {
        Object.keys(wpMap).forEach(fn => locked.add(cleanImageName(fn)));
        Object.values(wpMap).forEach(fn => { if (typeof fn === 'string') locked.add(cleanImageName(fn)); });
      }
    }
  } catch (e) {}

  // 2. From published posts in posts.json
  try {
    const postsFile = path.join(BASE_DIR, 'data/posts.json');
    if (fs.existsSync(postsFile)) {
      const posts = JSON.parse(fs.readFileSync(postsFile, 'utf8') || '[]');
      posts.forEach(p => {
        if (p.status === 'published' || p.wp_post_id) {
          if (p.imageUrl) locked.add(cleanImageName(p.imageUrl));
          if (p.secondaryImageUrl) locked.add(cleanImageName(p.secondaryImageUrl));
          if (p.featured_image) locked.add(cleanImageName(p.featured_image));
          if (p.content) {
            const matches = p.content.match(/!\[([^\]]*)\]\(([^)]+)\)/g) || [];
            matches.forEach(m => {
              const url = m.match(/\(([^)]+)\)/);
              if (url && url[1]) locked.add(cleanImageName(url[1]));
            });
          }
        }
      });
...
function getKeywordKho(keyword = '') {
  ...
  if (lowerKw.includes('công nghiệp') || lowerKw.includes('nhà xưởng') || lowerKw.includes('nhà máy') || lowerKw.includes('xí nghiệp') || lowerKw.includes('công suất lớn')) {
    return 'kho_2';
  }
...
function findMatchingCustomImage(keyword, index = 0, excludeUrls = []) {
  ...
  const excludeNames = new Set(excludeUrls.map(u => cleanImageName(u)));
  const filtered = pool.filter(m => !excludeNames.has(cleanImageName(m.url)));
...
function getTwoDistinctRotatedImages(keyword = '', recentUsedUrls = []) {
  ...
  // 3. Exclude locked WP / published images & recentUsedUrls
  const lockedSet = getLockedImagesSet();
  const recentSet = new Set((recentUsedUrls || []).map(u => cleanImageName(u)));

  // Level 1: NOT on WP/published AND NOT in recent batch
  let availablePool = candidateBase.filter(m => !lockedSet.has(cleanImageName(m.url)) && !recentSet.has(cleanImageName(m.url)));

  // Level 2: If pool has less than 2 available (all images in this Kho were used on WP at least once),
  // enter NEW CYCLE using LRU (Least-Recently-Used): items used furthest in the past come first!
  if (availablePool.length < 2) {
    const historyClean = getUsedHistory().map(u => cleanImageName(u));
    const sortedByLRU = [...candidateBase].sort((a, b) => {
      const idxA = historyClean.lastIndexOf(cleanImageName(a.url));
      const idxB = historyClean.lastIndexOf(cleanImageName(b.url));
      return idxA - idxB;
    });
    availablePool = sortedByLRU.filter(m => !recentSet.has(cleanImageName(m.url)));
    if (availablePool.length < 2) availablePool = sortedByLRU;
  }

  // Level 3: Fallback to candidateBase if needed
  if (availablePool.length < 2) {
    availablePool = candidateBase;
  }

  // 4. Shuffle availablePool to circulate images
  const shuffled = [...availablePool].sort(() => Math.random() - 0.5);
  const img1Obj = shuffled[0];
  let img2Obj = shuffled.find(m => cleanImageName(m.url) !== cleanImageName(img1Obj.url));

  if (!img2Obj) {
    const fallbackShuffled = [...candidateBase].sort(() => Math.random() - 0.5);
    img2Obj = fallbackShuffled.find(m => cleanImageName(m.url) !== cleanImageName(img1Obj.url)) || candidateBase[1] || candidateBase[0];
  }
  ...
  try {
    ...
    if (post.content && (img1 || img2)) {
      ...
      if (post.content.includes('[![')) {
        post.content = post.content.replace(/\[!\[([^\]]*)\]\(([^)]+)\)\]\(([^)]*)\)/g, (match, alt, oldImg, oldLink) => {
          const finalLink = oldLink && oldLink.startsWith('http') ? oldLink : targetUrl;
          if (count === 0 && img1) {
            count++;
            return `[![${alt || post.targetKeyword || 'Lọc nước'}](${img1})](${finalLink})`;
          } else if (count === 1 && img2) {
            count++;
            return `[![${alt || post.targetKeyword || 'Lọc nước'}](${img2})](${finalLink})`;
          }
          return match;
        });
      } else {
        post.content = post.content.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (match, alt) => {
          if (count === 0 && img1) {
            count++;
            return `[![${alt || post.targetKeyword || 'Lọc nước'}](${img1})](${targetUrl})`;
          } else if (count === 1 && img2) {
            count++;
            return `[![${alt || post.targetKeyword || 'Lọc nước'}](${img2})](${targetUrl})`;
          }
          return match;
        });
      }
...
function rotateMediaToAllPosts() {
  ...
  try {
    ...
    posts.forEach((post) => {
      ...
      const khoImages = mediaList.filter(m => categorizeImage(m) === postKho);
      const candidateBase = khoImages.length >= 2 ? khoImages : mediaList;

      // Filter candidates: exclude WP locked and batch assigned
      let pool = candidateBase.filter(m => !lockedSet.has(cleanImageName(m.url)) && !usedInBatch.has(cleanImageName(m.url)));
      // If fresh pool is exhausted, circulate by LRU (oldest used first)
      if (pool.length < 2) {
        const historyClean = getUsedHistory().map(u => cleanImageName(u));
        const sortedByLRU = [...candidateBase].sort((a, b) => {
          const idxA = historyClean.lastIndexOf(cleanImageName(a.url));
        ...
        pool = sortedByLRU.filter(m => !usedInBatch.has(cleanImageName(m.url)));
        if (pool.length < 2) pool = candidateBase;
      }

      const img1Obj = pool[0];
      const img2Obj = pool.find(m => cleanImageName(m.url) !== cleanImageName(img1Obj.url)) || pool[1] || pool[0];

      usedInBatch.add(cleanImageName(img1Obj.url));
      usedInBatch.add(cleanImageName(img2Obj.url));
      ...
      if (post.content) {
        ...
        if (post.content.includes('[![')) {
          post.content = post.content.replace(/\[!\[([^\]]*)\]\(([^)]+)\)\]\(([^)]*)\)/g, (match, alt, oldImg, oldLink) => {
            count++;
            const finalLink = oldLink && oldLink.startsWith('http') ? oldLink : targetUrl;
            if (count === 1) return `[![${alt || kw || 'Hệ thống lọc nước Hoa Sen'}](${img1Url})](${finalLink})`;
            if (count === 2) return `[![${alt || kw || 'Chi tiết kỹ thuật lọc nước Hoa Sen'}](${img2Url})](${finalLink})`;
            return match;
          });
        } else {
          post.content = post.content.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (match, alt) => {
            count++;
            if (count === 1) return `[![${alt || kw || 'Hệ thống lọc nước Hoa Sen'}](${img1Url})](${targetUrl})`;
            if (count === 2) return `[![${alt || kw || 'Chi tiết kỹ thuật lọc nước Hoa Sen'}](${img2Url})](${targetUrl})`;
            return match;
          });
        }
      }
      recordUsedImages([img1Url, img2Url]);
      updatedCount++;
    });
...
function rotateKhoMediaToPosts(khoId) {
  ...
  try {
    ...
    posts.forEach((post) => {
      ...
      // Filter candidates: exclude WP locked and batch assigned
      let pool = khoImages.filter(m => !lockedSet.has(cleanImageName(m.url)) && !usedInBatch.has(cleanImageName(m.url)));
      // If fresh pool is exhausted, circulate by LRU (oldest used first)
      if (pool.length < 2) {
        const historyClean = getUsedHistory().map(u => cleanImageName(u));
        const sortedByLRU = [...khoImages].sort((a, b) => {
          const idxA = historyClean.lastIndexOf(cleanImageName(a.url));
          const idxB = historyClean.lastIndexOf(cleanImageName(b.url));
          return idxA - idxB;
        });
        pool = sortedByLRU.filter(m => !usedInBatch.has(cleanImageName(m.url)));
        if (pool.length < 2) pool = khoImages;
      }
