...
function getTwoDistinctRotatedImages(keyword = '', recentUsedUrls = []) {
  ...
  // Level 2: If pool has less than 2 available, exclude only recent batch
  if (availablePool.length < 2) {
    availablePool = candidateBase.filter(m => !recentSet.has(cleanImageName(m.url)));
  }
...
function rotateMediaToAllPosts() {
  ...
  try {
    ...
    posts.forEach((post) => {
      ...
      if (pool.length < 2) {
        pool = candidateBase.filter(m => !usedInBatch.has(cleanImageName(m.url)));
...
function rotateKhoMediaToPosts(khoId) {
  ...
  try {
    ...
    posts.forEach((post) => {
      ...
      if (pool.length < 2) {
        pool = khoImages.filter(m => !usedInBatch.has(cleanImageName(m.url)));
