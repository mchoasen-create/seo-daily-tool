...
async function loadSchedulerStatus() {
  try {
    ...
    if (dispPendingGenCount) {
      if (ungeneratedCount > 0) {
        dispPendingGenCount.textContent = ungeneratedCount;
      } else if (schedulerStatusData.pendingCount > 0) {
        dispPendingGenCount.textContent = 'Xoay Vòng';
      } else {
        dispPendingGenCount.textContent = '0';
      }
    }
    if (dispNextGenKw) {
      if (schedulerStatusData.nextGenerateItem) {
        const isRot = schedulerStatusData.nextGenerateItem.isRotating;
        const kwText = schedulerStatusData.nextGenerateItem.keyword + (isRot ? ' (Xoay vòng)' : '');
        dispNextGenKw.textContent = kwText;
        dispNextGenKw.title = kwText;
        if (miniGenKw) miniGenKw.textContent = 'Từ khóa: ' + kwText;
      } else {
        dispNextGenKw.textContent = schedulerStatusData.pendingCount > 0 ? 'Tất cả bài trong hàng chờ đã soạn xong' : 'Hàng chờ trống';
        dispNextGenKw.title = '';
        if (miniGenKw) miniGenKw.textContent = schedulerStatusData.pendingCount > 0 ? 'Đã soạn xong bài' : 'Hàng chờ trống';
      }
    }
    if (dispNextGenTime) {
      dispNextGenTime.textContent = schedulerStatusData.nextGenerateItem
        ...
        : (ungeneratedCount === 0 ? 'Đã soạn xong hết' : formatDisplayDateTime(schedulerStatusData.nextGenerateTime));
    }
